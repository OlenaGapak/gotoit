    ...
    render() {
        ...
        data.simplified_reports.forEach(report => {
            if (
                ...
                if (this.state.selected_kind === report.kind && data.projects_unlocked_platforms.includes(report.platform)) {
                    put(report);
                }
            } else {
                if (this.state.selected_platform === report.platform && this.state.selected_kind === report.kind) {
                    put(report);
                }
            }
        });
                            ...
                            <tbody>
                                {table_data.map(row => (
                                    <tr key={row.id} className={row.is_player ? "table-success" : ""}>
                                        <th scope="row" className={row.is_player ? "table-success" : ""}>
                                            {row.top}
                                        </th>
